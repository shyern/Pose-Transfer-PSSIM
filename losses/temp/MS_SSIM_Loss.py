...
class MS_SSIM_Loss(torch.nn.Module):
    def __init__(self, base_window_size=11, size_average = True, max_val = 1):
    ...
    def _ssim(self, img1, img2, base_window_size=11, size_average = True):

        _, c, w, h = img1.size()
        window_size = min(w, h, base_window_size)
        sigma = 1.5 * window_size / base_window_size
    ...
    def ms_ssim(self, img1, img2, levels=5):
        ...
        for i in range(levels):
            ssim_map, mcs_map = self._ssim(img1, img2, self.base_window_size)
            msssim[i] = ssim_map
            mcs[i] = mcs_map
