...
# from losses.MS_SSIM_Loss import MS_SSIM_Loss
...
class TransferModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.input_BP2_mask_set = self.Tensor(nb, opt.scale_num, opt.BP_limbs_nc, size, size)
        ...
        if self.isTrain:
            ...
            elif opt.L1_type == 'PerceptualSSIM':
                self.criterionL1 = PerceptualSSIMLoss(opt.lambda_A, opt.lambda_B, opt.lambda_SSIM, opt.perceptual_layers, self.gpu_ids, opt.percep_is_l1, opt.win_size, opt.win_sigma)
            elif opt.L1_type == 'perL1':
                self.criterionL1 = PerceptualLoss(opt.lambda_B, opt.perceptual_layers, self.gpu_ids, opt.percep_is_l1)
            elif opt.L1_type == 'SSIM':
                # self.criterionSSIM = SSIM_Loss(window_size=opt.win_size, size_average=True)
                self.criterionSSIM = SSIM(win_size=opt.win_size,win_sigma=opt.win_sigma,data_range=1.0, size_average=True, channel=3)
            elif opt.L1_type == 'SSIM_plus_perL1':
                # self.criterionSSIM = SSIM_Loss(window_size=opt.window_size, size_average=True)
                self.criterionSSIM = SSIM(data_range=1.0, size_average=True, channel=3)
                self.criterionL1 = PerceptualLoss(opt.lambda_B, opt.perceptual_layers, self.gpu_ids, opt.percep_is_l1)
            elif opt.L1_type == 'partSSIM_plus_perL1_L1':
                self.criterionSSIM = Part_SSIM(data_range=1.0,size_average=True)
                # self.criterionL1 = PerceptualLoss(opt.lambda_B, opt.perceptual_layers, self.gpu_ids, opt.percep_is_l1)
                self.criterionL1 = L1_plus_perceptualLoss(opt.lambda_A, opt.lambda_B, opt.perceptual_layers, self.gpu_ids, opt.percep_is_l1)
            elif opt.L1_type == 'p_pSSIM_plus_perL1_L1':
                self.criterionSSIM = P_plus_P_SSIM(data_range=1.0, size_average=True, win_size=opt.win_size, win_sigma=opt.win_sigma)
                self.criterionL1 = L1_plus_perceptualLoss(opt.lambda_A, opt.lambda_B, opt.perceptual_layers, self.gpu_ids,
                                                      opt.percep_is_l1)
            elif opt.L1_type == 'SSIM_plus_perL1_L1':
                # self.criterionSSIM = SSIM_Loss(window_size=opt.window_size, size_average=True)
                self.criterionSSIM = SSIM(data_range=1.0, size_average=True, channel=3)
                self.criterionL1 = L1_plus_perceptualLoss(opt.lambda_A, opt.lambda_B, opt.perceptual_layers, self.gpu_ids, opt.percep_is_l1)
            # elif opt.L1_type == 'SSIM_plus_L1':
            #     self.criterionSSIM = SSIM(data_range=1.0, size_average=True, channel=3)
            #     self.criterionL1 = torch.nn.L1Loss()
            elif opt.L1_type == 'MS_SSIM':
                # self.criterionSSIM = MS_SSIM_Loss(base_window_size=opt.window_size, size_average=True)
                self.criterionSSIM = MS_SSIM(win_size=opt.win_size, win_sigma=0.9, data_range=1.0, size_average=True, channel=3, nonnegative_ssim=True)
            # elif opt.L1_type == 'Bodypart_SSIM':
            #     # self.criterionSSIM = Bodypart_SSIM(data_range=1.0, size_average=True, channel=3)
            #     self.criterionSSIM = Part_SSIM(data_range=1.0,size_average=True)
            # elif opt.L1_type == 'B_plus_P_SSIM':
            #     # self.criterionSSIM = Bodypart_SSIM(data_range=1.0, size_average=True, channel=3)
            #     self.criterionSSIM = B_plus_P_SSIM(data_range=1.0, size_average=True)
            # elif opt.L1_type == 'MS_PartSSIM':
            #     self.criterionSSIM = MS_Part_SSIM(data_range=1.0, size_average=True)
                # self.criterionL1 = L1_plus_perceptualLoss(opt.lambda_A, opt.lambda_B, opt.perceptual_layers, self.gpu_ids, opt.percep_is_l1)
            # elif opt.L1_type == 'Bodypart_SSIM_L1':
            #     # self.criterionSSIM = Bodypart_SSIM(win_size=opt.win_size, data_range=1.0, size_average=True, channel=3)
            #     self.criterionSSIM = Part_SSIM(data_range=1.0,size_average=True)
                self.criterionL1 = torch.nn.L1Loss()
            else:
    ...
    def backward_G(self):
        ...
        elif self.opt.L1_type == 'p_pSSIM_plus_perL1_L1':
            self.loss_ssim = (1 - self.criterionSSIM(self.fake_p2,
                                                     self.input_P2, self.input_BP2_mask_set)) * self.opt.lambda_SSIM
            losses = self.criterionL1(self.fake_p2, self.input_P2)
            self.loss_G_L1 = losses[0]+self.loss_ssim
            self.loss_originL1 = losses[1].item()
            self.loss_perceptual = losses[2].item()
            # self.loss_G_L1 = (1 - self.criterionSSIM(self.fake_p2,
            #                                          self.input_P2, self.input_BP2_mask_set)) * self.opt.lambda_SSIM + \
            #                  self.criterionL1(
            #                      self.fake_p2, self.input_P2)[0]
        elif self.opt.L1_type == 'PerceptualSSIM':
            losses = self.criterionL1(self.fake_p2, self.input_P2)
            self.loss_G_L1 = losses[0]
            self.loss_originL1 = losses[1].item()
            self.loss_perceptual_l1 = losses[2].item()
            self.loss_ssim = losses[3].item()
            self.loss_perceptual_feature = losses[4].item()
            # losses = self.criterionL1(self.fake_p2, self.input_P2)
            # self.loss_G_L1 = losses[0]
            # self.loss_originL1 = losses[1].item()
            # self.loss_perceptual = losses[2].item()
        elif self.opt.L1_type == 'perL1':
            self.loss_G_L1 = self.criterionL1(self.fake_p2, self.input_P2)
        elif self.opt.L1_type == 'SSIM':
            self.loss_G_L1 = (1-self.criterionSSIM(self.fake_p2, self.input_P2)) * self.opt.lambda_SSIM
        # elif self.opt.L1_type == 'SSIM_plus_perL1':
        #     self.loss_G_L1 = (1 - self.criterionSSIM(self.fake_p2,
        #                                              self.input_P2)) * self.opt.lambda_SSIM + self.criterionL1(
        #         self.fake_p2, self.input_P2)
        # elif self.opt.L1_type == 'partSSIM_plus_perL1_L1':
        #     self.loss_G_L1 = (1 - self.criterionSSIM(self.fake_p2,
        #                                              self.input_P2, self.input_BP2_mask_set)) * self.opt.lambda_SSIM + self.criterionL1(
        #         self.fake_p2, self.input_P2)[0]
        elif self.opt.L1_type == 'SSIM_plus_perL1_L1':
            self.loss_G_L1 = (1 - self.criterionSSIM(self.fake_p2,
                                                     self.input_P2)) * self.opt.lambda_SSIM + self.criterionL1(
                self.fake_p2, self.input_P2)[0]
        elif self.opt.L1_type == 'MS_SSIM':
            self.loss_G_L1 = - self.criterionSSIM(self.fake_p2, self.input_P2) * self.opt.lambda_SSIM
        # elif self.opt.L1_type == 'Bodypart_SSIM':
        #     self.loss_G_L1 = (1 - self.criterionSSIM(self.fake_p2, self.input_P2,
        #                                              self.input_BP2_mask_set)) * self.opt.lambda_SSIM
        # elif self.opt.L1_type == 'B_plus_P_SSIM':
        #     self.loss_G_L1 = (1-self.criterionSSIM(self.fake_p2, self.input_P2,
        #                                              self.input_BP2_mask_set)) * self.opt.lambda_SSIM
        # elif self.opt.L1_type == 'MS_PartSSIM':
        #     self.loss_G_L1 = (1-self.criterionSSIM(self.fake_p2, self.input_P2, self.input_BP2_mask_set)) * self.opt.lambda_SSIM
            # self.loss_G_L1 = (1 - self.criterionSSIM(self.fake_p2,
            #                                          self.input_P2, self.input_BP2_mask_set)) * self.opt.lambda_SSIM + \
            #                  self.criterionL1(
            #                      self.fake_p2, self.input_P2)[0]
        # elif self.opt.L1_type == 'Bodypart_SSIM_L1':
        #     self.loss_G_L1 = (1 - self.criterionSSIM(self.fake_p2, self.input_P2,
        #                                              self.input_BP2_mask_set)) * self.opt.lambda_SSIM+self.criterionL1(self.fake_p2, self.input_P2) * self.opt.lambda_A
        elif self.opt.L1_type == 'origin':
    ...
    def get_current_errors(self):
        ...
        if self.opt.L1_type == 'l1_plus_perL1' or self.opt.L1_type == 'p_pSSIM_plus_perL1_L1' or self.opt.L1_type == 'PerceptualSSIM':
            ret_errors['origin_L1'] = self.loss_originL1
            ret_errors['perceptual'] = self.loss_perceptual
            # ret_errors['ssim'] = self.loss_ssim.item()
            # ret_errors['origin_L1'] = self.loss_originL1
            # ret_errors['perceptual_l1'] = self.loss_perceptual_l1
            # ret_errors['loss_ssim'] = self.loss_ssim
            # ret_errors['loss_perceptual_feature'] = self.loss_perceptual_feature
